...
export default {
  ...
  methods: {
    ...
    startCooperate() {
      ...
      const userName = userInfo?.user?.name || `用户${this.userId}`;
      const userColor = ['#409EFF', '#67C23A', '#E6A23C', '#F56C6C', '#909399'][
        Math.floor(Math.random() * 5)
      ];

      this.postToIframe({
        type: 'start_cooperate',
        roomId: String(this.mindMapId),
        userId: this.userId,
        userName: userName,
        userColor: userColor,
      });
    ...
    handleIframeMessage(event) {
      ...
      const { type, roomId, changes, uids, userInfo, nodeIds } = data;

      switch (type) {
        case 'cooperate_connect':
          this.cooperateRoomId = roomId;
          // 通知 iframe 已连接
          this.postToIframe({ type: 'connected' });
          break;

        case 'cooperate_disconnect':
          this.cooperateRoomId = null;
          break;

        case 'cooperate_update':
          // 转发到 WebSocket
          this.sendCooperateToWs({ type: 'update', changes });
          break;

        case 'cooperate_delete':
          this.sendCooperateToWs({ type: 'delete', uids });
          break;

        case 'cooperate_full_sync':
          this.sendCooperateToWs({ type: 'full_sync', data: data.data });
          break;

        case 'cooperate_awareness':
          this.sendCooperateToWs({ type: 'awareness', userInfo, nodeIds });
          break;
      }
    },
    ...
    postToIframe(data) {
      ...
      if (iframe?.contentWindow) {
        iframe.contentWindow.postMessage({ source: 'cooperate-parent', ...data }, '*');
      }
    },

    // 发送协同消息到 WebSocket
    sendCooperateToWs(message) {
      if (!this.socket?.isConnected) return;
      if (this.online.count <= 1 && message.type !== 'full_sync') return;
    ...
    // 处理从 WebSocket 收到的协同消息，转发给 iframe
    handleCooperateMessage(message) {
      // 根据消息子类型转发给 iframe
      const subType = message.type === 'cooperate' ? message.subType || message.msgType : message.type;

      switch (subType) {
        case 'update':
          this.postToIframe({ type: 'update', changes: message.changes });
          break;
        case 'delete':
          this.postToIframe({ type: 'delete', uids: message.uids });
          break;
        case 'full_sync':
          this.postToIframe({ type: 'full_sync', data: message.data });
          break;
        case 'init':
          this.postToIframe({ type: 'init', data: message.data });
          break;
        case 'awareness':
          this.postToIframe({
            type: 'awareness',
          ...
          break;
        case 'awareness_sync':
          this.postToIframe({ type: 'awareness_sync', data: message.data });
          break;
        case 'awareness_remove':
          this.postToIframe({ type: 'awareness_remove', clientId: message.clientId });
          break;
        default:
          // 兼容直接传递的消息格式
          if (message.changes) {
            this.postToIframe({ type: 'update', changes: message.changes });
          } else if (message.uids) {
            this.postToIframe({ type: 'delete', uids: message.uids });
    ...
    init() {
      ...
      if (iframe) {
        ...
        // 获取当前用户信息
        const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
        const userName = encodeURIComponent(userInfo?.user?.name || `用户${this.userId}`);
        // 传递协同所需的参数
        const cooperateParams = `&roomId=${this.mindMapId}&userId=${this.userId}&userName=${userName}`;
        const newSrc = `/mindmap/mindmap.html?id=${this.mindMapId}&t=${Date.now()}${isPublicParam}${cooperateParams}`;
