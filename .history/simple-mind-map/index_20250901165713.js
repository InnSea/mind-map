...
class MindMap {
  ...
  updateData(data) {
    if (this._isUpdating) {
      // 如果正在更新，保存最新的待更新数据
      this._pendingUpdate = { type: 'update', data }
      return
    }
    this._executeUpdate('update', data)
  }
  ...
  setFullData(data) {
    if (this._isUpdating) {
      // 如果正在更新，保存最新的待更新数据
      this._pendingUpdate = { type: 'full', data }
      return
    }
    this._executeUpdate('full', data)
  }

  _executeUpdate(type, data) {
  ...
  const onComplete = () => {
    ...
    if (this._pendingUpdate) {
      ...
      this._pendingUpdate = null
      this._executeUpdate(pending.type, pending.data)
    }
  }
