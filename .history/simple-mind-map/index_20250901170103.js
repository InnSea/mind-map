...
class MindMap {
  ...
  updateData(data) {
    this._lastUpdateData = { type: 'update', data }
    this._scheduleUpdate()
  }

  // 修改setFullData方法
  setFullData(data) {
    this._lastUpdateData = { type: 'full', data }
    this._scheduleUpdate()
  }
  ...
  _scheduleUpdate() {
    ...
    this._updateTimer = setTimeout(() => {
      const { type, data } = this._lastUpdateData
      this._lastUpdateData = null

      if (type === 'update') {
        /
        const processedData = this.handleData(data)
        this.emit('before_update_data', processedData)
        this.renderer.setData(processedData)
        this.render()
        this.command.addHistory()
        this.emit('update_data', processedData)
      } else if (type === 'full') {
        // 原setFullData逻辑
        if (data.root) {
