...
class MindMap {
  ...
  constructor(opt = {}) {
    ...
    */
    ...
    // 数据更新队列机制
    this._updateQueue = [] // 更新队列
    this._isProcessingQueue = false // 是否正在处理队列
  ...
  // 队列处理方法
  _processUpdateQueue() {
    if (this._isProcessingQueue || this._updateQueue.length === 0) {
      return
    }
    ...
    // 获取队列中最新的更新请求
    const latestUpdate = this._updateQueue[this._updateQueue.length - 1]
    // 清空队列，只处理最新的更新
    this._updateQueue = []

    try {
      // 执行实际的更新操作
      this._executeUpdate(latestUpdate)
    } catch (error) {
      console.error('队列处理错误:', error)
      this.opt.errorHandler && this.opt.errorHandler(ERROR_TYPES.DATA_CHANGE_DETAIL_EVENT_ERROR, error)
    } finally {
      this._isProcessingQueue = false
      
      // 如果在处理过程中又有新的更新请求，继续处理
      if (this._updateQueue.length > 0) {
        this._scheduleQueueProcess()
      }
    }
  }
  ...
  // 执行具体的更新操作
  _executeUpdate(updateRequest) {
    const { type, data } = updateRequest
    
    if (type === 'updateData') {
      this._doUpdateData(data)
    } else if (type === 'setFullData') {
      this._doSetFullData(data)
    }
  }
  ...
  _doUpdateData(data) {
    ...
    if (!data || typeof data !== 'object') {
      console.warn('updateData: 无效的数据格式', data)
      return
    }
    ...
    try {
      ...
      console.error('updateData 执行错误:', error)
      this.opt.errorHandler && this.opt.errorHandler(ERROR_TYPES.DATA_CHANGE_DETAIL_EVENT_ERROR, error)
    }
  }

  // 实际执行 setFullData 的逻辑
  _doSetFullData(data) {
    // 数据验证
    if (!data || typeof data !== 'object') {
      console.warn('setFullData: 无效的数据格式', data)
      return
    }
    ...
    try {
      ...
      console.error('setFullData 执行错误:', error)
      this.opt.errorHandler && this.opt.errorHandler(ERROR_TYPES.DATA_CHANGE_DETAIL_EVENT_ERROR, error)
    }
  }
