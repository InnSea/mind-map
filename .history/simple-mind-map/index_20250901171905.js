...
class MindMap {
  ...
  _processUpdateQueue() {
    ...
    try {
      ...
      // 如果有resolve方法，调用它
      if (latestUpdate.resolve) {
        latestUpdate.resolve()
      }
    } catch (error) {
      // 如果有reject方法，调用它；否则使用errorHandler
      if (latestUpdate.reject) {
        latestUpdate.reject(error)
      } else {
        this.opt.errorHandler(error, ERROR_TYPES.DATA_CHANGE_DETAIL_EVENT_ERROR)
      }
    } finally {
