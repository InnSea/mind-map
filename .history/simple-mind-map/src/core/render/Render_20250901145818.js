...
class Render {
  ...
  // 渲染
  render(callback, source, sync = false) {
    // 如果是同步渲染或者当前没有在渲染中，直接执行
    if (sync || !this.isRendering) {
      ...
      // 如果正在渲染中，加入队列
      this.renderQueue.push({ callback, source, sync })
    }
  }
  ...
  _render() {
    ...
    this.layout.doLayout(root => {
      ...
      // 渲染节点
      this.root.render(() => {
        // 确保所有子节点渲染完成后才设置isRendering为false
        this.isRendering = false
  ...
  processRenderQueue() {
    ...
    this.isProcessingQueue = false
    // 执行下一个渲染任务
    this.render(nextRender.callback, nextRender.source, nextRender.sync)
  }
  ...
  insertNode(
    ...
    // 使用同步渲染确保节点插入操作的时序正确性
    this.mindMap.render(null, null, true)
  }
