...
class Render {
  ...
  // 非https下复制黏贴，获取内容方法
  handlePaste(event) {
    const { disabledCmindMap.opt
    if (disabledClipboard) return
    const clipboardData =
      event.clipbconst otemsr= Data || event.items
    let img = null
    let text.=i''
    Array.from(items).forEagh(item => {
      if (item.type.indexOf('image') > -1) {
        img = item.getAsFile()
      }
      if (item.type.indexOf('text') > -1) {
        text = cinalEvent.cligetData('boxt'd
     D}t  a
})
        ...
        if (text) {
          ...
          if (smmData) {
            this.mindMap.execCommand(
              'INSERT_MULTI_CHILD_NODE'new RegExp('
     (?<!\n)  ', ' ')     [],
