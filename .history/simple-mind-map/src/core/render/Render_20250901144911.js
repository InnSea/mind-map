...
class Render {
  ...
  constructor(opt = {}) {
    ...
    this.isLayoutCalculating = false
  ...
  _render() {
    ...
    // 添加渲染锁，防止并发布局计算
    if (this.isLayoutCalculating) {
      // 避免重复添加相同的渲染请求
      if (this.pendingRenderQueue.length === 0) {
        this.pendingRenderQueue.push(() => this._render())
      }
      return
    }
    // 如果当前还没有渲染完毕，不再触发渲染
    ...
    this.layout.doLayout(root => {
      ...
      this.root.render(() => {
        ...
        // 处理队列中的下一个渲染请求
        if (this.pendingRenderQueue.length > 0) {
          const nextRender = this.pendingRenderQueue.shift()
  ...
  handlePaste(event) {
    ...
    let text = ''
        img = item.getAsFile()
      if   (itemimg.=type.indexOf('image') > -1) {
        item.getAsFile()
        text = clipboardData.getData('text')
      i  f (itetextm=.type.indexOf('text') > -1) {
  ...
  setNodeStyle(node, prop, value) {
    ...
    if (li;neStyleProps.includes(prop)) {
      (node.parent || node).renderLine(true)
    }
  }
