  ...
  bind() {
    ...
    this.mindMap.event.on('mousewheel', (e, dirs, event, isTouchPad) => {
      ...
      if (
        ...
        let stepY = 0
        if (isTouchPad) {
          // 如果是触控板，使用触控板滑动距离并应用灵敏度调节
          // 触控板通常比鼠标滚轮更敏感，所以需要降低灵敏度
          const touchpadSensitivity = 0.3 // 可调节的触控板灵敏度因子
          const threshold = 5 // 阈值，过滤掉微小的意外滚动
          
          // 只有当滚动值大于阈值时才应用，避免微小的意外滚动
          stepX = Math.abs(e.wheelDeltaX) > threshold ? Math.abs(e.wheelDeltaX) * touchpadSensitivity : 0
          stepY = Math.abs(e.wheelDeltaY) > threshold ? Math.abs(e.wheelDeltaY) * touchpadSensitivity : 0
        } else {
