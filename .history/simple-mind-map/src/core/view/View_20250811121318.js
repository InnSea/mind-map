...
function animateScroll() {
  ...
  this.translateXY(velocityX, velocityY);
  velocityX *= 0.85; // 惯性衰减
  velocityY *= 0.85;
    ...
    this.mindMap.event.on('mousewheel', (e, dirs, event, isTouchPad) => {
      ...
      if (
        ...
        if (
          ...
          dirs = dirs.filter(
            dir => ![CONSTANTS.DIR.LEFT, CONSTANTS.DIR.RIGHT].includes(dir)
          )
        }
        switch (true) {
        ...
        // 平移逻辑
        if (isTouchPad) {
          const HORIZONTAL_DEADZONE = 2
          let dx = e.deltaX || e.wheelDeltaX || 0
          let dy = e.deltaY || e.wheelDeltaY || 0

          // 过滤微小横向滚动
          if (Math.abs(dx) < HORIZONTAL_DEADZONE) dx = 0
          // 如果垂直滚动幅度明显大于水平，忽略水平
          if (Math.abs(dy) > Math.abs(dx) * 2) dx = 0

          velocityX += dx * translateRatio
          velocityY += dy * translateRatio
