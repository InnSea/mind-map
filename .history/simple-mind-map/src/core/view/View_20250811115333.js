...
class View {
    ...
    this.mindMap.event.on('mousewheel', (e, dirs, event, isTouchPad) => {
      ...
      if (
        ...
        if (isTouchPad) {
          ...
          // 鼠标滚轮处理（保持原有逻辑）
          let stepX = stepY = mousewheelMoveStep
          let mx = 0
          let my = 0
          // 上移
  ...
  handleTouchpadScroll(e, dirs, translateRatio) {
    ...
    // 根据方向计算delta值
    if (dirs.includes(CONSTANTS.DIR.LEFT)) {
      deltaX = Math.abs(e.wheelDeltaX) * this.touchpadScrolling.sensitivity
    }
    if (dirs.includes(CONSTANTS.DIR.RIGHT)) {
      deltaX = -Math.abs(e.wheelDeltaX) * this.touchpadScrolling.sensitivity
    }
    if (dirs.includes(CONSTANTS.DIR.UP)) {
      deltaY = Math.abs(e.wheelDeltaY) * this.touchpadScrolling.sensitivity
    }
    if (dirs.includes(CONSTANTS.DIR.DOWN)) {
      deltaY = -Math.abs(e.wheelDeltaY) * this.touchpadScrolling.sensitivity
    }
