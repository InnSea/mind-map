    ...
    this.mindMap.event.on('mousewheel', (e, dirs, event, isTouchPad) => {
      ...
      if (
        ...
        if (isTouchPad) {
          ...
          const rawDX = e.wheelDeltaX || 0
          const rawDY = e.wheelDeltaY || 0
          // 当本次滚动仅有上下方向意图时，若横向漂移小于阈值则忽略
          const verticalOnly =
          ...
          const H_THRESHOLD = this.mindMap?.opt?.touchpadHorizontalIgnoreThreshold ?? 6
          const dx = verticalOnly && Math.abs(rawDX) < H_THRESHOLD ? 0 : rawDX
          stepX = Math.abs(dx)
          stepY = Math.abs(rawDY)
        } else {
