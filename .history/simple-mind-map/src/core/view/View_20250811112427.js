...
class View {
  ...
  constructor(opt = {}) {
    ...
    this.firstDrag = true
    // 平滑移动相关
    this.smoothMoveAnimationId = null
  ...
  bind() {
    ...
    this.mindMap.event.on('mousewheel', (e, dirs, event, isTouchPad) => {
      ...
      if (
        ...
        if (isTouchPad) {
          stepX = this.calculateDynamicStep(e.wheelDeltaX || e.deltaX, true)
          stepY = this.calculateDynamicStep(e.wheelDeltaY || e.deltaY, true)
        } else {
          // 鼠标滚轮：根据滚动速度动态调整步长
          const deltaValue = Math.abs(e.deltaY || e.wheelDelta)
          stepX = stepY = this.calculateDynamicStep(deltaValue, false)
        }
        ...
        const finalMx = mx * translateRatio
        const finalMy = my * translateRatio

        // 根据滚动频率选择移动方式
        if (timeDelta < 50) {
          // 快速连续滚动时使用即时移动，避免动画堆积
          this.translateXY(finalMx, finalMy)
        } else {
          // 单次或慢速滚动时使用平滑移动
          const duration = Math.min(200, Math.max(100, Math.sqrt(stepX + stepY) * 20))
          this.smoothTranslateXY(finalMx, finalMy, duration)
        }
      }
    })
  ...
  //  平滑移动x,y方向（带动量和缓动）
  smoothTranslateXY(deltaX, deltaY, duration = 300) {
    if (this.smoothMoveAnimationId) {
      cancelAnimationFrame(this.smoothMoveAnimationId)
    }
    ...
    const animate = (currentTime) => {
      ...
      if (progress < 1) {
        this.smoothMoveAnimationId = requestAnimationFrame(animate)
      } else {
        this.smoothMoveAnimationId = null
        this.emitEvent('translate')
      }
  ...
  //  计算动态移动步长
  calculateDynamicStep(deltaValue, isTouchPad = false) {
    const { mousewheelMoveStep } = this.mindMap.opt
    
    if (isTouchPad) {
      // 触控板使用实际滑动距离，但限制最大值
      return Math.min(Math.abs(deltaValue), mousewheelMoveStep * 3)
    } else {
      // 鼠标滚轮根据滚动速度动态调整
      const speedFactor = Math.min(Math.abs(deltaValue) / 100, 3)
      return mousewheelMoveStep * (0.5 + speedFactor)
    }
  }
