...
class View {
  ...
  constructor(opt = {}) {
    ...
    // 触控板平滑滚动相关属性
    this.touchpadScrolling = {
      velocityX: 0,
      velocityY: 0,
      isScrolling: false,
      animationId: null,
      lastTime: 0,
      friction: 0.85, // 摩擦系数，控制减速
      sensitivity: 0.8, // 触控板灵敏度调节
  ...
  bind() {
    ...
    this.mindMap.event.on('mousewheel', (e, dirs, event, isTouchPad) => {
      ...
      if (
        ...
        if (isTouchPad) {
          // 触控板平滑滚动处理
          this.handleTouchpadScroll(e, dirs, translateRatio)
        } else {
          // 鼠标滚轮处理（保持原有逻辑）
          let stepX = mousewheelMoveStep
          let stepY = mousewheelMoveStep
          let mx = 0
          let my = 0
          // 上移
  ...
  //  触控板平滑滚动处理
  handleTouchpadScroll(e, dirs, translateRatio) {
    const currentTime = performance.now()
    const deltaTime = currentTime - this.touchpadScrolling.lastTime
    this.touchpadScrolling.lastTime = currentTime

    // 计算滚动速度，并应用灵敏度调节
    let deltaX = 0
    let deltaY = 0
    
    // 直接使用wheelDelta值，保持原有的正负号和比例
    if (e.wheelDeltaX !== undefined) {
      deltaX = -e.wheelDeltaX * this.touchpadScrolling.sensitivity
    }
    if (e.wheelDeltaY !== undefined) {
      deltaY = -e.wheelDeltaY * this.touchpadScrolling.sensitivity
    }
    ...
    deltaX = Math.max(-maxVelocity, Math.min(maxVelocity, deltaX))
    deltaY = Math.max(-maxVelocity, Math.min(maxVelocity, deltaY))

    // 更新速度（加权平均，使滚动更平滑）
    const smoothingFactor = 0.3
    this.touchpadScrolling.velocityX = this.touchpadScrolling.velocityX * (1 - smoothingFactor) + deltaX * smoothingFactor
    this.touchpadScrolling.velocityY = this.touchpadScrolling.velocityY * (1 - smoothingFactor) + deltaY * smoothingFactor

    // 开始平滑滚动动画
    this.startTouchpadScrollAnimation(translateRatio)
  }

  //  开始触控板平滑滚动动画
  startTouchpadScrollAnimation(translateRatio) {
    if (this.touchpadScrolling.isScrolling) {
      return // 如果已经在滚动，不重复启动
    }

    this.touchpadScrolling.isScrolling = true
    
    const animate = () => {
      // 应用当前速度进行移动
      if (Math.abs(this.touchpadScrolling.velocityX) > this.touchpadScrolling.minVelocity ||
          Math.abs(this.touchpadScrolling.velocityY) > this.touchpadScrolling.minVelocity) {
        
        this.translateXY(
          this.touchpadScrolling.velocityX * translateRatio,
          this.touchpadScrolling.velocityY * translateRatio
        )

        // 应用摩擦力减速
        this.touchpadScrolling.velocityX *= this.touchpadScrolling.friction
        this.touchpadScrolling.velocityY *= this.touchpadScrolling.friction

        // 继续动画
        this.touchpadScrolling.animationId = requestAnimationFrame(animate)
      } else {
        // 速度太小，停止动画
        this.stopTouchpadScrollAnimation()
      }
    }
  ...
  //  停止触控板平滑滚动动画
  stopTouchpadScrollAnimation() {
    if (this.touchpadScrolling.animationId) {
      cancelAnimationFrame(this.touchpadScrolling.animationId)
      this.touchpadScrolling.animationId = null
    }
    this.touchpadScrolling.isScrolling = false
    this.touchpadScrolling.velocityX = 0
    this.touchpadScrolling.velocityY = 0
  }
