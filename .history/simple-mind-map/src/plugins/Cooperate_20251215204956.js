...
class Cooperate {
  ...
  _onMessage(event) {
    ...
    switch (data.type) {
      case 'start_cooperate':
        // 父页面通知开始协同
        this._startCooperate(data)
        break
  ...
  // 开始协同
  _startCooperate(data) {
    const { roomId, userId, userName, userColor, userAvatar } = data
    ...
    this.setUserInfo({
      ...
      color: userColor || '#409EFF',
      avatar: userAvatar || ''
    })

    // 初始化当前数据
    if (!this.currentData && this.mindMap.getData) {
      const mindMapData = this.mindMap.getData()
      if (mindMapData) {
        this.initData(mindMapData)
      }
    }
