// 生成用户头像 SVG（与项目 icons.js 格式一致）
function generateAvatarSvg(name, color) {
  const displayName = name.length >= 2 ? name.slice(-2) : name
  // 使用与项目一致的 1024x1024 viewBox，圆形背景 + 文字
  return `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 1024 1024"><circle cx="512" cy="512" r="512" fill="${color}"></circle><text x="512" y="512" font-family="Arial, sans-serif" font-size="420" font-weight="bold" fill="#FFFFFF" text-anchor="middle" dominant-baseline="central">${displayName}</text></svg>`
}
...
export async function loadUserIconList() {
  ...
  try {
    ...
    const response = await fetch(
      '/classtorch-api/auth/user/list?page=1&size=1000'
    )
