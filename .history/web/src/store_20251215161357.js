...
const store = new Vuex.Store({
  ...
  mutations: {
    ...
    // 设置用户图标加载状态
    setUserIconLoading(state, data) {
      state.userIconLoading = data
    }
  },
  actions: {
    async loadUserIconList({ commit, state }) {
      if (state.userIconLoading || state.userIconList.length > 0) {
        return state.userIconList
      }
      commit('setUserIconLoading', true)
      try {
        ...
        commit('setUserIconLoading', false)
      }
    }
  }
})
