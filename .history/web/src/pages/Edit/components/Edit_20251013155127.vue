...
import externalEnvironment from '@/mixin/externalEnvironment'
  ...
  methods: {
    ...
    cooperateEdit() {
      if (this.mindMap.cooperate && this.$route.query.userName) {
        // 获取协作提供者配置
        const providerConfig = this.getCooperateProviderConfig();
        this.mindMap.cooperate.setProvider(null, providerConfig);
        
        // 获取协作用户配置
        const userConfig = this.getCooperateUserConfig();
        this.mindMap.cooperate.setUserInfo({
          id: Math.random(),
          name: this.$route.query.userName,
          color: userConfig.colors[Math.floor(Math.random() * userConfig.colors.length)],
          avatar: Math.random() > 0.5 ? userConfig.defaultAvatar : ''
        });
      }
    },
