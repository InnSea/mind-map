  ...
  methods: {
    ...
    cooperateEdit() {
      if (this.mindMap.cooperate && this.$route.query.userName) {
        // 优先从externalEnvironment获取协作编辑配置
        let cooperateConfig = this.getCooperateConfig()
        
        // 如果没有配置，尝试从takeOverAppMethods获取（向后兼容）
        if (!cooperateConfig && window.takeOverAppMethods && window.takeOverAppMethods.getCooperateConfig) {
          cooperateConfig = window.takeOverAppMethods.getCooperateConfig()
        }
        ...
        const config = cooperateConfig || defaultConfig

        console.log('协作编辑配置:', config);
        
        this.mindMap.cooperate.setProvider(null, config)
        this.mindMap.cooperate.setUserInfo({
          id: Math.random(),
          name: cooperateConfig?.userName || this.$route.query.userName,
