...
<script>
import { nodeIconList as _nodeIconList, getUserIconListSync, loadUserIconList, getFullNodeIconList } from 'simple-mind-map/src/svg/icons'
...
export default {
    ...
    methods: {
        ...
        async show(node, nodeIcon) {
            this.node = node
            this.iconType = nodeIcon.split('_')[0]
            this.iconName = nodeIcon.split('_')[1]
            this.nodeIconList = node.getData('icon') || []
            
            // 如果是用户头像类型且未加载，先加载
            if (this.iconType === 'user_avatar' && !getUserIconListSync()) {
                await loadUserIconList()
            }
            
            const allIconList = getFullNodeIconList(icon)
            const iconGroup = allIconList.find((item) => item.type === this.iconType)
            if (iconGroup && iconGroup.list) {
                this.iconList = [...iconGroup.list]
            } else {
                this.iconList = []
            }
