...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      // 使用预加载的用户图标
      const fullIconList = [...this.preloadedUserIcons, ...icon]
      ...
      try {
        ...
        this.mindMap.updateConfig({
          iconList: [...customIcons, ...icon]
        })
        this.$store.commit('setDynamicIconList', customIcons)
      } catch (e) {
        console.error('加载用户图标失败', e)
      }
      // 解析url中的文件
      if (hasFileURL) {
