...
import { getUserIconListSync, getUserIconList } from '@/config/userIcon'

const baseIconList = [..._nodeIconList, ...icon]

function getAllIconList() {
  const userIconList = getUserIconListSync()
  if (userIconList) {
    return [userIconList, ...baseIconList]
  }
  return baseIconList
}
...
export default {
    ...
    methods: {
        ...
        async show(node, icon) {
            ...
            // 获取图标列表，如果是用户头像类型则确保已加载
            let allIconList = getAllIconList()
            if (this.iconType === 'user_avatar' && !getUserIconListSync()) {
                await getUserIconList()
                allIconList = getAllIconList()
            }
