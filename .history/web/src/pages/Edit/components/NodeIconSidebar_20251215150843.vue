...
import { nodeIconList, loadUserIconList, getFullNodeIconList, getUserIconListSync } from 'simple-mind-map/src/svg/icons'
...
export default {
  ...
  methods: {
    ...
    handleAppInited(mindMap) {
      ...
      // 如果用户头像已加载，更新 mindMap 的 iconList
      this.updateMindMapIconList()
    },

    updateMindMapIconList() {
      if (this.mindMap && this.userIconsLoaded) {
        const userIconList = getUserIconListSync()
        if (userIconList) {
          this.mindMap.opt.iconList = [userIconList, ...icon]
        }
      }
    },
    ...
    // 加载用户头像图标
    async loadUserIcons() {
      if (this.userIconsLoaded) return
      try {
        await loadUserIconList()
        this.nodeIconList = mergerIconList(getFullNodeIconList(icon))
        this.userIconsLoaded = true
        // 更新 mindMap 的 iconList
        this.updateMindMapIconList()
      } catch (error) {
