  ...
  methods: {
    ...
    cooperateEdit() {
      if (this.mindMap.cooperate && this.$route.query.userName) {
        // 获取外部配置的WebSocket地址，如果没有则使用默认配置
        const cooperateConfig = window.takeOverAppMethods?.getCooperateConfig?.() || {
          signalingList: ['ws://localhost:4444'],
          roomName: 'mind-room'
        }
        
        this.mindMap.cooperate.setProvider(null, cooperateConfig)
