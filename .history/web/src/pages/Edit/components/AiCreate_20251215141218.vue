    ...
    >
      <div class="inputBox">
        <el-input
          type="textarea"
          :rows="3"
          :placeholder="$t('ai.aiCreatePartPlaceholder')"d
          v-model="aiPartInput"
        >
        </el-input>
        <div class="contextInfo">
          <span class="label">{{ $t('ai.currentNode') }}：</span>
          <span class="value">{{ currentNodeText }}</span>
        </div>
      </div>
      <div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    // 显示AI续写弹窗
      showAiCreatePartDialog(node) {
        ...
        this.aiPartInput = this.$t('ai.aiCreatePartDefaultInput')
        this.currentNodeText = getStrWithBrFromHtml(node.getData('text'))
    ...
    getNodeContext(node) {
      ...
      while (currentNode) {
        const text = getStrWithBrFromHtml(currentNode.getData('text'))
        context.path.unshift(text)
        currentNode = currentNode.parent
      }

      if (node.parent && node.parent.children) {
        context.siblings = node.parent.children
          .filter(child => child !== node)
          .map(child => getStrWithBrFromHtml(child.getData('text')))
      }

      if (node.children && node.children.length > 0) {
        context.existingChildren = node.children.map(child =>
          getStrWithBrFromHtml(child.getData('text'))
        )
      }
    ...
    // 构建AI续写的提示词
    buildAiCreatePartPrompt() {
      const context = this.getNodeContext(this.beingCreatePartNode)
      const currentNodeText = getStrWithBrFromHtml(
        this.beingCreatePartNode.getData('text')
      )

      const pathDesc = context.path.join(' > ')

      let siblingsDesc = ''
      if (context.siblings.length > 0) {
        siblingsDesc = this.$t('ai.siblingsInfo') + context.siblings.join('、')
      }

      let existingChildrenDesc = ''
      if (context.existingChildren.length > 0) {
        existingChildrenDesc =
          this.$t('ai.existingChildrenInfo') +
          context.existingChildren.join('、')
      }

      const userInstruction = this.aiPartInput.trim()
      const userInstructionDesc = userInstruction
        ? this.$t('ai.userInstructionInfo') + userInstruction
        : ''

      const prompt = `${this.$t('ai.aiCreatePartPromptPrefix')}

${this.$t('ai.pathInfo')}${pathDesc}

${this.$t('ai.currentNodeInfo')}${currentNodeText}
${siblingsDesc ? '\n' + siblingsDesc : ''}${existingChildrenDesc ? '\n' + existingChildrenDesc : ''}${userInstructionDesc ? '\n' + userInstructionDesc : ''}

${this.$t('ai.aiCreatePartPromptSuffix')}`
    ...
    async aiCreatePart() {
      try {
        ...
        const prompt = this.buildAiCreatePartPrompt()
